...
function displayData ()
{
    data.forEach(function (an) {
			...
			para.textContent = an.description.substring(0, 50)
			title.textContent = an.title.text
			const genres = document.createElement("p")
			genres.textContent = an.genres.filter(function (g) {
				return g == "Comedy"
			})
			genres.style.width = "max-content"
			link.href = an.title.link
			link.target = "_blank"
			anime.append(title, para, genres)
			link.appendChild(anime)
...
button.addEventListener('click', displayData)/* event listners*/
